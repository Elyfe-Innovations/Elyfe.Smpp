...
public abstract class PDU
{
  ...
  {
    var header = new PDUHeader(CommandType.GenericNack, vHeader.SequenceNumber);
    header.ErrorCode = errorCode;
    var gNack = (GenericNack)CreatePDU(header, vSmppEncodingService);
    return gNack;
  }
  ...
  public virtual byte[] GetBytes()
  {
    ...
    var tlvData = vTlv.GetBytes(vSmppEncodingService);
    var length = 16;
    length += bodyData == null ? 0 : bodyData.Length;
    length += tlvData == null ? 0 : tlvData.Length;
    vHeader.CommandLength = (uint)length;
    var buffer = new ByteBuffer(length); //Allocate buffer with enough capacity
  ...
  public void SetBodyData(ByteBuffer buffer)
  {
    ...
    {
      PDUParseException.WrapAndThrow(ex);
    }
  }
  ...
  internal static string DecodeString(ByteBuffer buffer, int length, SmppEncodingService smppEncodingService)
  {
    try
    {
      var value = smppEncodingService.GetStringFromBytes(buffer.Remove(length));
      return value;
    }
    catch (ArgumentException ex)
    {
  ...
  {
    return GetOptionalParamByte<byte>(tag);
  }
