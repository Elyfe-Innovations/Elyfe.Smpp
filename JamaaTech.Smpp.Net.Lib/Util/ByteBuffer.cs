...
public sealed class ByteBuffer
{
  ...
  private int Reserved => vArrayBuffer.Length - vNextPosition;
  ...
  public byte Remove()
  {
    ...
    var result = vArrayBuffer[vNextPosition - vLength];
    vLength--;
    return result;
  }
  ...
  {
    var newBuffer = new byte[vArrayBuffer.Length];
    if (Length > 0)
  ...
  {
    var newBufferSize = ((int)(increment / vCapacity) + 1) * vCapacity;
    //Hold the current array buffer
    var currentBuffer = vArrayBuffer;
    //Allocate a new array buffer
    vArrayBuffer = new byte[vArrayBuffer.Length + newBufferSize];
    var startIndex = vNextPosition - vLength;
    var length = vLength;
    vNextPosition = 0;
    vLength = 0;
