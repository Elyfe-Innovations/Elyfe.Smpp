...
{
  private static GSMEncoding gsm = new();
  ...
  public static byte[] GetBytes(string str)
  {
    ...
    var buffer = new ByteBuffer(str.Length);
    foreach (var @char in str) //For each charactor in the string
    {
      var @byte = (byte)@char; //Get its Latin1 byte order index
      var index = vDefaultReverseTable[@byte]; //Get its SMSC Default byte order index
  ...
  public static string GetString(byte[] bytes)
  {
    ...
    for (var index = 0; index < bytes.Length; ++index)
    {
      ...
      if (@byte == 0x1b) //This might be an escape sequence
        ...
        {
          var @extChar = GetExtendedChar(bytes[index + 1]);
      ...
      var @char = vDefaultForwardTable[@byte]; //Get charactor corresponding to this index
  ...
  private static void BuildTable()
  {
    ...
    var chars = new string(vDefaultForwardTable, 0, vDefaultForwardTable.Length);
    var index = 0;
    for (; index < byte.MaxValue; ++index) vDefaultReverseTable[index] = 128;
